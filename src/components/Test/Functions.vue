  ...
  <v-container fluid class="app-page">
    <div class="app-page-header">
      <div>
        ...
        <div class="app-page-subtitle">Online / active sessions from auth_sessions</div>
      </div>
    ...
    <v-card class="app-card" elevation="0">
      ...
      >
        ...
        <template #item.client="{ item }">
          <v-chip size="x-small" variant="tonal" color="info">{{ item.client }}</v-chip>
        </template>
        ...
        <template #item.actions="{ item }">
          <v-tooltip :text="`Greet ${item.fullName || item.deviceId}`" location="top">
      ...
      <div class="d-flex align-center pa-4 ga-2 flex-wrap">
        <v-btn color="primary" variant="flat" prepend-icon="mdi-clock-out" :loading="loading.clockOut" @click="onClockOut">
          Clock Out
        </v-btn>
        <v-spacer />
        <v-btn color="error" variant="tonal" prepend-icon="mdi-logout" :loading="loading.logOut" @click="onLogOut">
          Log Out
        </v-btn>
      </div>

      <div class="px-4 pb-4">
        <v-alert v-if="lastError" type="error" variant="tonal" density="compact" rounded="lg">
          {{ lastError }}
        </v-alert>
        <v-alert v-if="lastInfo" type="info" variant="tonal" density="compact" rounded="lg" class="mt-2">
          {{ lastInfo }}
        </v-alert>
      </div>
    </v-card>
